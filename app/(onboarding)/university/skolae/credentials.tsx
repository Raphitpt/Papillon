...
import { GesAuthenticationToken, ProfileService } from "ges-api-react-native";
import { GesAPI } from "ges-api-react-native";
import { router } from "expo-router";

const ANIMATION_DURATION = 100;

export default function SkolaeCredentials() {
  ...
  async function handleLogin(username: string, password: string) {
    ...
    try {
      const response: GesAuthenticationToken = await GesAPI.login(username, password);
      ...
      const service: ServiceAccount = {
        ...
        serviceId: Services.SKOLAE,
        createdAt: (new Date()).toISOString(),
        updatedAt: (new Date()).toISOString(),
      };

      const profile = await ProfileService.getProfile(response);
      ...
      store.addAccount({
        ...
        className: profile.student_id ?? undefined,
        services: [service],
        createdAt: (new Date()).toISOString(),
        updatedAt: (new Date()).toISOString(),
        customisation: {
          profilePicture: profile._links.photo.href,
          subjects: {},
        }
      });
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {t("ONBOARDING_LOGIN_CREDENTIALS")} {"Skolae"}
          </Typography>
        </Stack>
      </View>
